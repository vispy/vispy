...
float unpack_interpolate(sampler2D kernel, vec2 uv, vec2 pixel) {
    float u = uv.x;
    float v = uv.y;
    float uf = fract(u / pixel.x);

    float d0 = unpack(texture2D(kernel, vec2(u - uf * pixel.x, v)));
    float d1 = unpack(texture2D(kernel, vec2(u - (uf - 1) * pixel.x, v)));
    return mix(d0, d1, uf);
}

vec4
filter1D_radius1(sampler2D kernel, float index, float x, vec2 pixel, vec4 c0, vec4 c1)
{
    float w, w_sum = 0.0;
    vec4 r = vec4(0.0,0.0,0.0,0.0);
    w = unpack_interpolate(kernel, vec2(0.000000+(x/1.0), index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c0 * w;
    w = unpack_interpolate(kernel, vec2(1.000000-(x/1.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c1 * w;
    return r;
}
...
filter2D_radius1(sampler2D texture, sampler2D kernel, float index, vec2 uv, vec2 pixel)
{
    ...
    vec4 t0 = filter1D_radius1(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(0,0)*pixel),
        texture2D( texture, texel + vec2(1,0)*pixel));
    vec4 t1 = filter1D_radius1(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(0,1)*pixel),
        texture2D( texture, texel + vec2(1,1)*pixel));
    return filter1D_radius1(kernel, index, f.y, pixel, t0, t1);
}

vec4
filter1D_radius2( sampler2D kernel, float index, float x, vec2 pixel, vec4 c0, vec4 c1, vec4 c2, vec4 c3)
{
    float w, w_sum = 0.0;
    vec4 r = vec4(0.0,0.0,0.0,0.0);
    w = unpack_interpolate(kernel, vec2(0.500000+(x/2.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c0 * w;
    w = unpack_interpolate(kernel, vec2(0.500000-(x/2.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c2 * w;
    w = unpack_interpolate(kernel, vec2(0.000000+(x/2.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c1 * w;
    w = unpack_interpolate(kernel, vec2(1.000000-(x/2.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c3 * w;
    return r;
}
...
filter2D_radius2(sampler2D texture, sampler2D kernel, float index, vec2 uv, vec2 pixel)
{
    ...
    vec2 f = fract(texel);
    texel = (texel-fract(texel)+vec2(0.001,0.001))*pixel;
    vec4 t0 = filter1D_radius2(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-1,-1)*pixel),
        texture2D( texture, texel + vec2(0,-1)*pixel),
        texture2D( texture, texel + vec2(1,-1)*pixel),
        texture2D( texture, texel + vec2(2,-1)*pixel));
    vec4 t1 = filter1D_radius2(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-1,0)*pixel),
        texture2D( texture, texel + vec2(0,0)*pixel),
        texture2D( texture, texel + vec2(1,0)*pixel),
        texture2D( texture, texel + vec2(2,0)*pixel));
    vec4 t2 = filter1D_radius2(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-1,1)*pixel),
        texture2D( texture, texel + vec2(0,1)*pixel),
        texture2D( texture, texel + vec2(1,1)*pixel),
        texture2D( texture, texel + vec2(2,1)*pixel));
    vec4 t3 = filter1D_radius2(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-1,2)*pixel),
        texture2D( texture, texel + vec2(0,2)*pixel),
        texture2D( texture, texel + vec2(1,2)*pixel),
        texture2D( texture, texel + vec2(2,2)*pixel));
    return filter1D_radius2(kernel, index, f.y, pixel, t0, t1, t2, t3);
}

vec4
filter1D_radius3(sampler2D kernel, float index, float x, vec2 pixel, vec4 c0, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5)
{
    float w, w_sum = 0.0;
    vec4 r = vec4(0.0,0.0,0.0,0.0);
    w = unpack_interpolate(kernel, vec2(0.666667+(x/3.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c0 * w;
    w = unpack_interpolate(kernel, vec2(0.333333-(x/3.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c3 * w;
    w = unpack_interpolate(kernel, vec2(0.333333+(x/3.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    ...
    w = w*kernel_scale + kernel_bias;
    r += c4 * w;
    w = unpack_interpolate(kernel, vec2(0.000000+(x/3.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c2 * w;
    w = unpack_interpolate(kernel, vec2(1.000000-(x/3.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c5 * w;
    return r;
}
...
filter2D_radius3(sampler2D texture, sampler2D kernel, float index, vec2 uv, vec2 pixel)
{
    ...
    vec2 f = fract(texel);
    texel = (texel-fract(texel)+vec2(0.001,0.001))*pixel;
    vec4 t0 = filter1D_radius3(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-2,-2)*pixel),
        texture2D( texture, texel + vec2(-1,-2)*pixel),
        texture2D( texture, texel + vec2(0,-2)*pixel),
        texture2D( texture, texel + vec2(1,-2)*pixel),
    ...
    vec4 t1 = filter1D_radius3(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-2,-1)*pixel),
        texture2D( texture, texel + vec2(-1,-1)*pixel),
        texture2D( texture, texel + vec2(0,-1)*pixel),
        texture2D( texture, texel + vec2(1,-1)*pixel),
    ...
    vec4 t2 = filter1D_radius3(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-2,0)*pixel),
        texture2D( texture, texel + vec2(-1,0)*pixel),
        texture2D( texture, texel + vec2(0,0)*pixel),
        texture2D( texture, texel + vec2(1,0)*pixel),
    ...
    vec4 t3 = filter1D_radius3(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-2,1)*pixel),
        texture2D( texture, texel + vec2(-1,1)*pixel),
        texture2D( texture, texel + vec2(0,1)*pixel),
        texture2D( texture, texel + vec2(1,1)*pixel),
        texture2D( texture, texel + vec2(2,1)*pixel),
        texture2D( texture, texel + vec2(3,1)*pixel));
    vec4 t4 = filter1D_radius3(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-2,2)*pixel),
        texture2D( texture, texel + vec2(-1,2)*pixel),
        texture2D( texture, texel + vec2(0,2)*pixel),
        texture2D( texture, texel + vec2(1,2)*pixel),
        texture2D( texture, texel + vec2(2,2)*pixel),
        texture2D( texture, texel + vec2(3,2)*pixel));
    vec4 t5 = filter1D_radius3(kernel, index, f.x, pixel,
    ...
    return filter1D_radius3(kernel, index, f.y, pixel, t0, t1, t2, t3, t4, t5);
}

vec4
filter1D_radius4( sampler2D kernel, float index, float x, vec2 pixel, vec4 c0, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5, vec4 c6, vec4 c7)
{
    float w, w_sum = 0.0;
    vec4 r = vec4(0.0,0.0,0.0,0.0);
    w = unpack_interpolate(kernel, vec2(0.750000+(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c0 * w;
    w = unpack_interpolate(kernel, vec2(0.250000-(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c4 * w;
    w = unpack_interpolate(kernel, vec2(0.500000+(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c1 * w;
    w = unpack_interpolate(kernel, vec2(0.500000-(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    ...
    w = w*kernel_scale + kernel_bias;
    r += c6 * w;
    w = unpack_interpolate(kernel, vec2(0.000000+(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c3 * w;
    w = unpack_interpolate(kernel, vec2(1.000000-(x/4.0),index), pixel);
    w = w*kernel_scale + kernel_bias;
    r += c7 * w;
    return r;
}
...
filter2D_radius4(sampler2D texture, sampler2D kernel, float index, vec2 uv, vec2 pixel)
{
    ...
    vec2 f = fract(texel);
    texel = (texel-fract(texel)+vec2(0.001,0.001))*pixel;
    vec4 t0 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,-3)*pixel),
        texture2D( texture, texel + vec2(-2,-3)*pixel),
        texture2D( texture, texel + vec2(-1,-3)*pixel),
        texture2D( texture, texel + vec2(0,-3)*pixel),
    ...
    vec4 t1 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,-2)*pixel),
        texture2D( texture, texel + vec2(-2,-2)*pixel),
        texture2D( texture, texel + vec2(-1,-2)*pixel),
        texture2D( texture, texel + vec2(0,-2)*pixel),
    ...
    vec4 t2 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,-1)*pixel),
        texture2D( texture, texel + vec2(-2,-1)*pixel),
        texture2D( texture, texel + vec2(-1,-1)*pixel),
        texture2D( texture, texel + vec2(0,-1)*pixel),
    ...
    vec4 t3 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,0)*pixel),
        texture2D( texture, texel + vec2(-2,0)*pixel),
        texture2D( texture, texel + vec2(-1,0)*pixel),
        texture2D( texture, texel + vec2(0,0)*pixel),
    ...
    vec4 t4 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,1)*pixel),
        texture2D( texture, texel + vec2(-2,1)*pixel),
        texture2D( texture, texel + vec2(-1,1)*pixel),
        texture2D( texture, texel + vec2(0,1)*pixel),
        ...
        texture2D( texture, texel + vec2(3,1)*pixel),
        texture2D( texture, texel + vec2(4,1)*pixel));
    vec4 t5 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,2)*pixel),
        texture2D( texture, texel + vec2(-2,2)*pixel),
        texture2D( texture, texel + vec2(-1,2)*pixel),
        texture2D( texture, texel + vec2(0,2)*pixel),
        ...
        texture2D( texture, texel + vec2(3,2)*pixel),
        texture2D( texture, texel + vec2(4,2)*pixel));
    vec4 t6 = filter1D_radius4(kernel, index, f.x, pixel,
        texture2D( texture, texel + vec2(-3,3)*pixel),
        texture2D( texture, texel + vec2(-2,3)*pixel),
        texture2D( texture, texel + vec2(-1,3)*pixel),
        texture2D( texture, texel + vec2(0,3)*pixel),
        ...
        texture2D( texture, texel + vec2(3,3)*pixel),
        texture2D( texture, texel + vec2(4,3)*pixel));
    vec4 t7 = filter1D_radius4(kernel, index, f.x, pixel,
    ...
    return filter1D_radius4(kernel, index, f.y, pixel, t0, t1, t2, t3, t4, t5, t6, t7);
}
