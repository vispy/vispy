...
def test_wrappers():
    ...
    with Canvas():
        ...
        assert_raises(ValueError, gloo.set_blend_color, (0.0, 0.0))  # bad color
        ...
        gloo.clear((1.0, 1.0, 1.0, 1.0), 0.5, 1)
        gloo.flush()
        gloo.finish()
        # check some results
...
def test_read_pixels():
    ...
    # Create vertices
    vPosition = np.array(
        [[-1, 1, 0.0], [0, 1, 0.5], [-1, 0, 0.0], [0, 0, 0.5]],  # For drawing a square to top left
        np.float32,
    )
