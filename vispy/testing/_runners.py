...
def _get_import_dir():
    ...
    if (
        op.isfile(op.join(up_dir, "setup.py"))
        and op.isdir(op.join(up_dir, "vispy"))
        and op.isdir(op.join(up_dir, "examples"))
    ):
        dev = True
    else:
        dev = False
...
def _unit(mode, extra_arg_string="", coverage=False):
    ...
    if mode == "nobackend":
        ...
        stdout, stderr, invalid = run_subprocess(
            [sys.executable, "-c", 'import vispy.app; vispy.app.use_app("%s"); exit(0)' % mode],
            return_code=True,
        )
        if invalid:
            stdout = stdout + "\n" + stderr
    ...
    if len(msg) > 0:
        ...
        print(msg)
    sys.stdout.flush()
    return_code = run_subprocess(cmd, return_code=True, cwd=cwd, env=env, stdout=None, stderr=None)[
        2
    ]
    if return_code:
...
def _docs():
    ...
    if not dev:
        warnings.warn(
            "Docstring test imports Vispy from"
    ...
    for fi, fname in enumerate(fnames):
        ...
        if retcode or len(stderr.strip()) > 0:
            ...
            if "ImportError: " in stderr:
                ...
                ext = "\n" + _line_sep + "\n"
                fails.append(
                    "X%sExample %s failed (%s):%s%s%s" % (ext, root_name, retcode, ext, stderr, ext)
                )
                print(fails[-1])
        else:
            print("✓")
...
def test(label="full", extra_arg_string="", coverage=False):
    ...
    if label not in known_types + backend_names:
        raise ValueError(
            "label must be one of %s, or a backend name %s, "
