...
def initShaders():
    ...
    Shaders = [
        ...
        # this is useful for viewing shells that enclose a volume (such as
        # isosurfaces)
        ...
        # This means that the colors will change depending on how the view is
        # rotated
...
class Shader(object):
    ...
    def shader(self):
        if self.compiled is None:
            try:
                self.compiled = shaders.compileShader(
                    self.code,
                    self.shaderType)
                ...
                if len(exc.args) == 3:
                    ...
                    code = [errNums[i] +
                            ...
                            (maxlen -
                             len(errNums[i])) +
                            line for i, line in enumerate(code)]
                    err = err + '\n'.join(code)
                    raise Exception(err)
                else:
                    raise
...
class ShaderProgram(object):
    ...
    def program(self):
        if self.prog is None:
            try:
                compiled = [s.shader()
                            for s in self.shaders]  # compile all shaders
        ...
        if len(self.shaders) > 0 and self.program() != -1:
            ...
            try:
                ...
                for uniformName, data in self.uniformData.items():
                    ...
                    if loc == -1:
                        raise Exception(
                            'Could not find uniform variable "%s"' %
                            uniformName)
...
class HeightColorShader(ShaderProgram):
    ...
    def __enter__(self):
        ...
        # pick any-old binding point. (there are a limited number of these
        # per-program
        bindPoint = 1
