...
from vispy.testing import requires_application, SkipTest, run_tests_if_main, assert_raises, IS_CI
...
def test_context_properties():
    ...
    if (
        "pyqt5" in a.backend_name.lower()
        or "pyqt6" in a.backend_name.lower()
        or "pyside2" in a.backend_name.lower()
        or "pyside6" in a.backend_name.lower()
    ):
        pytest.xfail(
            "Context sharing is not supported in PyQt5, PyQt6, PySide2, or PySide6 at this time."
        )
...
def test_context_sharing():
    ...
    with Canvas() as c1:
        ...
        if (
            "pyqt5" in c1.app.backend_name.lower()
            or "pyqt6" in c1.app.backend_name.lower()
            or "pyside2" in c1.app.backend_name.lower()
            or "pyside6" in c1.app.backend_name.lower()
        ):
            pytest.xfail(
                "Context sharing is not supported in "
                "PyQt5, PyQt6, PySide2, or PySide6 at this time."
            )
