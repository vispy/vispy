...
def _test_callbacks(canvas):
    ...
    if backend_name.lower() == "pyglet":
        ...
        event = namedtuple("event", ["type", "window", "motion", "button", "wheel", "key"])
...
def test_capability():
    ...
    if c.app.backend_name == "Glfw" and c.app.backend_module.glfw.__version__ == (3, 3, 1):
...
def test_abstract():
    ...
    for fun in (
        app._vispy_get_backend_name,
        app._vispy_process_events,
        app._vispy_run,
        app._vispy_quit,
    ):
