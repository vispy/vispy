...
class AxisVisual(CompoundVisual):
    ...
    def __init__(
        ...
        self._line = LineVisual(method="gl", width=axis_width, antialias=True, color=axis_color)
        self._ticks = LineVisual(
            method="gl", width=tick_width, connect="segments", antialias=True, color=tick_color
        )
        ...
        CompoundVisual.__init__(self, [self._line, self._text, self._ticks, self._axis_label_vis])
        if pos is not None:
            self.pos = pos
        self.domain = domain
...
class Ticker(object):
    ...
    def _get_tick_positions(self, major_tick_fractions, minor_tick_fractions):
        ...
        axislabel_vector = (
            direction * (self.axis.major_tick_length + self.axis.axis_label_margin) / doc_len
        )
    ...
    def _get_tick_frac_labels(self):
        ...
        if self.axis.scale_type == "linear":
            ...
            labels = [label for li, label in enumerate(labels) if use_mask[li]]
