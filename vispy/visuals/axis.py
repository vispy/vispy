...
from ..util.axis import Formatter, ScalarFormatter
...
class AxisVisual(CompoundVisual):
    ...
    def __init__(
        ...
        self.ticker = Ticker(
            self, ScalarFormatter(), anchors=anchors, tick_fmt_func=tick_fmt_func
        )
        ...
        self._line = LineVisual(
            method="gl", width=axis_width, antialias=True, color=axis_color
        )
        self._ticks = LineVisual(
            method="gl",
            width=tick_width,
            connect="segments",
            antialias=True,
        ...
        self._axis_label_vis = TextVisual(font_size=axis_font_size, color=text_color)
        CompoundVisual.__init__(
            self, [self._line, self._text, self._ticks, self._axis_label_vis]
        )
        if pos is not None:
            self.pos = pos
        self.domain = domain
...
class Ticker(object):
    ...
    def __init__(
        self, axis: AxisVisual, formatter: Formatter, anchors=None, tick_fmt_func=None
    ):
        self.axis = axis
        if not isinstance(formatter, Formatter):
            raise ValueError("Ticker expected an instance of `Formatter`.")
    ...
    def _get_tick_frac_labels(self):
        ...
        if self.axis.scale_type == "linear":
            ...
            self.formatter.set_locs(major)
            labels = [self.formatter(x) for x in major]
...
def _density_max(k, m):
    return 2 - (k - 1.0) / (m - 1.0) if k >= m else 1.0
            ...
            while k < n_max:
                ...
                while z < float("infinity"):
                    ...
                    for start in range(int(min_start), int(max_start) + 1):
                        ...
                        leg = 1.0  # _legibility(lmin, lmax, lstep)
