...
from vispy.testing import requires_application, TestingCanvas, run_tests_if_main, IS_CI
...
def test_image_clims_and_gamma(
    input_dtype, texture_format, num_channels, clim_on_init, data_on_init
):
    ...
    is_16int_cpu_scaled = (
        np.dtype(input_dtype).itemsize >= 2
...
def test_image_nan_single_band(texture_format):
    ...
    with TestingCanvas(size=size[::-1], bgcolor=(0, 1, 0)) as c:
        Image(data, cmap="grays", texture_format=texture_format, parent=c.scene)
        rendered = c.render()
...
def test_image_equal_clims(texture_format, num_channels):
    ...
    with TestingCanvas(size=size[::-1], bgcolor="w") as c:
        Image(
            data, cmap="viridis", texture_format=texture_format, clim=(128.0, 128.0), parent=c.scene
        )
        rendered = c.render()[..., :3]
...
def test_image_vertex_updates():
    ...
    with TestingCanvas(size=size, bgcolor="w") as c:
        ...
        with mock.patch.object(
            image, "_build_vertex_data", wraps=image._build_vertex_data
...
def test_image_interpolation():
    ...
    with TestingCanvas(size=size[::-1], bgcolor="w") as c:
        ...
        image = Image(data=data, cmap="grays", parent=view.scene)
        ...
        assert np.allclose(render[right], white)
        assert np.allclose(
            render[center], gray, atol=5
        )  # we just want gray, this is not quantitative
...
def test_image_set_data_different_dtype():
    ...
    with TestingCanvas(size=size[::-1], bgcolor="w") as c:
        ...
        view.camera = PanZoomCamera((0, 0, 2, 1))
        image = Image(data=data, cmap="grays", clim=[0, 127], parent=view.scene)
