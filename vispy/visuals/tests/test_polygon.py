...
from vispy.testing import requires_application, requires_scipy, TestingCanvas, run_tests_if_main
...
def test_square_draw():
    ...
    pos = np.array([[-0.5, 0.5, 0], [0.5, 0.5, 0], [0.5, -0.5, 0], [-0.5, -0.5, 0]])
    with TestingCanvas() as c:
        polygon = visuals.Polygon(pos=pos, color=(1, 0, 0, 1), parent=c.scene)
        polygon.transform = transforms.STTransform(scale=(50, 50), translate=(50, 50))

        assert_image_approved(c.render(), "visuals/square1.png")

        polygon.parent = None
        polygon = visuals.Polygon(
            pos=pos, color=(1, 0, 0, 1), border_color=(1, 1, 1, 1), parent=c.scene
        )
        polygon.transform = transforms.STTransform(scale=(50, 50), translate=(50, 50))
        ...
        polygon.parent = None
        polygon = visuals.Polygon(pos=pos, border_color=(1, 1, 1, 1), parent=c.scene)
        polygon.transform = transforms.STTransform(scale=(50, 50), translate=(50, 50))
...
def test_rectangle_draw():
    ...
    pos = np.array([[-0.1, 0.5, 0], [0.1, 0.5, 0], [0.1, -0.5, 0], [-0.1, -0.5, 0]])
    with TestingCanvas() as c:
        polygon = visuals.Polygon(pos=pos, color=(1, 1, 0, 1), parent=c.scene)
        polygon.transform = transforms.STTransform(scale=(200.0, 25), translate=(50, 50))

        assert_image_approved(c.render(), "visuals/rectangle1.png")

        polygon.parent = None
        polygon = visuals.Polygon(
            pos=pos, color=(1, 1, 0, 1), border_color=(1, 0, 0, 1), parent=c.scene
        )
        polygon.transform = transforms.STTransform(scale=(200.0, 25), translate=(50, 50))
        ...
        polygon.parent = None
        polygon = visuals.Polygon(
            pos=pos, border_color=(1, 0, 0, 1), border_width=1, parent=c.scene
        )
        polygon.transform = transforms.STTransform(scale=(200.0, 25), translate=(50, 49))

        assert_image_approved(c.render(), "visuals/rectangle3.png", min_corr=0.7)
...
def test_reactive_draw():
    ...
    with TestingCanvas() as c:
        ...
        assert_image_approved(c.render(), "visuals/reactive_polygon3.png", min_corr=0.8)
