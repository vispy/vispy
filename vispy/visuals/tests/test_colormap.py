...
def test_colormap():
    ...
    with TestingCanvas(size=size, bgcolor="w") as c:
        ...
        image = Image(
            cmap=Colormap(colors=["k", "w", "r"], controls=[0.0, 0.1, 1.0]),
            clim="auto",
            parent=c.scene,
        )
        image.set_data(data)
...
def test_colormap_discrete():
    ...
    with TestingCanvas(size=size, bgcolor="w") as c:
        ...
        data = idata.reshape((size[0], size[1]))
        image = Image(
            cmap=Colormap(colors=["r", "g", "b"], interpolation="zero"), clim="auto", parent=c.scene
        )
        image.set_data(data)
...
def test_colormap_discrete_nu():
    ...
    with TestingCanvas(size=size, bgcolor="w") as c:
        ...
        image = Image(
            cmap=Colormap(
                np.array([[0, 0.75, 0], [0.75, 0.25, 0.5]]), [0.0, 0.25, 1.0], interpolation="zero"
            ),
            clim="auto",
            parent=c.scene,
        )
