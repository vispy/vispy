...
def test_reactive_draw():
    ...
    with TestingCanvas() as c:
        colorbar = create_colorbar(pos=(50, 50), size=(60, 4), orientation="top")
        c.draw_visual(colorbar)

        colorbar.cmap = "ice"
        assert_image_approved(c.render(), "visuals/colorbar/reactive_cmap.png")

        colorbar.clim = (-20, 20)
        assert_image_approved(c.render(), "visuals/colorbar/reactive_clim.png")

        colorbar.label.text = "new label"
        assert_image_approved(c.render(), "visuals/colorbar/reactive_label.png")

        colorbar.ticks[0].color = "red"
        colorbar.ticks[1].color = "blue"
        assert_image_approved(c.render(), "visuals/colorbar/reactive_ticks.png")

        colorbar.border_width = 0
        assert_image_approved(c.render(), "visuals/colorbar/reactive_border_width.png")

        colorbar.border_width = 5
        colorbar.border_color = "red"
        assert_image_approved(c.render(), "visuals/colorbar/reactive_border_color.png")
...
def test_attributes():
    ...
    with TestingCanvas():
        ...
        with raises(ValueError):
            create_colorbar(pos=(50, 50), size=(60, 4), orientation="top-invalid")
...
def test_colorbar_label_as_textvisual():
    with TestingCanvas() as c:
        ...
        colorbar = create_colorbar(pos=(50, 50), size=(60, 4), orientation="top", label=label)
        colorbar.cmap = "ice"
