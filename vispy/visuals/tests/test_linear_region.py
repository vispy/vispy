...
def test_linear_region_vertical_horizontal():
    ...
    for is_vertical, reference_image in [
        (True, "linear_region1.png"),
        ...
        with TestingCanvas() as c:
            ...
            region = visuals.LinearRegion(
                pos=pos, color=[0.0, 1.0, 0.0, 0.5], vertical=is_vertical, parent=c.scene
            )
...
def test_linear_region_gradient():
    ...
    # Definition of the color of the region
    color2 = np.array(
        [[0.0, 1.0, 0.0, 0.5], [1.0, 0.0, 0.0, 0.75], [0.0, 0.0, 1.0, 1.0]], dtype=np.float32
    )
    # Expected internal color buffer
    color2_expected = np.array(
        [color2[0], color2[0], color2[1], color2[1], color2[2], color2[2]], dtype=np.float32
    )
