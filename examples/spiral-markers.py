...
a_bg_color  = np.ones((n,4)) * (1,1,1,1)
a_fg_color  = np.ones((n,4)) * (0,0,0,1)
a_size      = np.random.uniform(2,12,(n,1)).astype(np.float32)
a_linewidth = np.ones((n,1))
u_antialias = .75
...
for i in range(500):
    theta += dtheta
    x = 2*256      + 2*radius*np.cos(theta)
    y = 2*256+2*32 + 2*radius*np.sin(theta)
    r = 10.1-i*0.02;
    radius -= 0.45
    a_position[i] = x,y,0
    a_size[i] = 2*2*r
...
for i in range(39):
    ...
    x = 2*20+2*i*12.5 - 2*r
    y = 2*16
    a_position[500+i] = x,y,0
    a_size[500+i] = 2*2*r
    a_linewidth[500+i] = thickness
...
void main (void) {
    ...
    v_fg_color.a = min(a_linewidth, v_fg_color.a);
    v_linewidth = max(a_linewidth, 1.0);
...
class Canvas(app.Canvas):
    def __init__(self):
        ...
        self.geometry = (0,0,2*512,2*512+2*32)
        ...
        self.program.attributes['a_linewidth']  = a_linewidth
        ...
        self.projection = ortho(0, 512, 0, 512, -1, 1)
