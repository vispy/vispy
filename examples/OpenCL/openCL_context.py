...
if __name__ == '__main__':
    imgs = [numpy.random.randint(0, 65000, N ** 2).reshape((N, N)).astype(numpy.uint16) for i in range(10) ]
    c = Canvas(N)
#    c.measure_fps(1)
    c.show()
    c.init_gl()
    ...
    while True:
        ...
        for i in imgs:
            c.new_image(i)
        t1 = time.time()
        print("Rendering at %4.2f fps (average of %i)" % (len(imgs) / (t1 - t0), len(imgs)))
    app.run()
