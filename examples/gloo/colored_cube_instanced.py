...
class Canvas(app.Canvas):
    def __init__(self):
        app.Canvas.__init__(
            self, size=(512, 512), title="Colored instanced cube", keys="interactive"
        )

        # Build cube data
        verts, idxs, _ = create_cube()
        vertices = VertexBuffer(verts)
        self.indices = IndexBuffer(idxs)

        instance_shift = VertexBuffer(
            ((np.random.rand(100, 3) - 0.5) * 50).astype(np.float32), divisor=1
        )
