...
class WigglyBar(app.Canvas):
    def __init__(
        ...
        piv_x_y_px = np.asarray(
            (self.pivot_loc_px * np.sin(self.theta), -1 * self.pivot_loc_px * (np.cos(self.theta)))
        )
        ...
        self.text = visuals.TextVisual(
            "0:00.00", color="white", pos=[50, 250, 0], anchor_x="left", anchor_y="bottom"
        )
        ...
        self.rod = visuals.BoxVisual(
            width=self.px_len / 40, height=self.px_len / 40, depth=self.px_len, color="white"
        )
        ...
        self.mass = visuals.BoxVisual(
            width=self.px_len / 4, height=self.px_len / 8, depth=self.px_len / 4, color="white"
        )
    ...
    def _set_up_system(
        ...
        self.mass_loc = np.asarray(
            [self.x_is_0 + self.x * self.px_per_m, self.d2 * self.px_per_m * np.cos(self.theta)]
        )
...
class SetupWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.pivot_chk = QtWidgets.QCheckBox("Show pivot point")
        ...
        self.parameter_groupbox = QtWidgets.QGroupBox("System Parameters")
        self.conditions_groupbox = QtWidgets.QGroupBox("Initial Conditions")
        self.display_groupbox = QtWidgets.QGroupBox("Display Parameters")
