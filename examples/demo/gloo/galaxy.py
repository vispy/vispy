...
def make_arm(n, angle):
    R = np.linspace(10, 450 + 50 * np.random.uniform(0.5, 1.0), n)
    R += 40 * np.random.normal(0, 2.0, n) * np.linspace(1, 0.1, n)
    T = angle + np.linspace(0, 2.5 * np.pi, n) + np.pi / 6 * np.random.normal(0, 0.5, n)
    S = 8 + 2 * np.abs(np.random.normal(0, 1, n))
    S *= np.linspace(1, 0.85, n)
    P = np.zeros((n, 3), dtype=np.float32)
    X, Y, Z = P[:, 0], P[:, 1], P[:, 2]
    X[...] = R * np.cos(T)
    Y[...] = R * np.sin(T) * 1.1
    D = np.sqrt(X * X + Y * Y)
    Z[...] = 8 * np.random.normal(0, 2 - D / 512.0, n)
    X += (D * np.random.uniform(0, 1, n) > 250) * (0.05 * D * np.random.uniform(-1, 1, n))
    Y += (D * np.random.uniform(0, 1, n) > 250) * (0.05 * D * np.random.uniform(-1, 1, n))
    Z += (D * np.random.uniform(0, 1, n) > 250) * (0.05 * D * np.random.uniform(-1, 1, n))
...
class Canvas(app.Canvas):
    def __init__(self):
        ...
        data = np.zeros(
            n, [("a_position", np.float32, 3), ("a_size", np.float32), ("a_dist", np.float32)]
        )
        ...
        self.program["u_size"] = 5.0 / self.translate
        ...
        gloo.set_state(
            depth_test=False, blend=True, blend_func=("src_alpha", "one"), clear_color="black"
        )
