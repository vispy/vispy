...
for i in range(16):
    n = i * 2 + 1
    y += (4.0 / np.pi) * (1.0 / n) * np.sin(n * np.pi * x / L)
    if n in plot_nvals:
        tmp_line = fig[0, 0].plot((x, y), color=colors[plot_nvals.index(n)], width=2)
...
labels = [
    vp.Label("n=%d" % plot_nvals[i], color=colors[i], anchor_x="left")
