...
try:
    ...
    warnings.warn(
        "Lasso example requires matplotlib for more accurate selection. "
        "Falling back to numpy based selection."
    )
    path = None

LASSO_COLOR = (1, 0.1, 0.1)
...
indexes = np.random.normal(size=NUMBER_POINT, loc=centers.shape[0] / 2, scale=centers.shape[0] / 3)
...
def points_in_polygon(polygon, pts):
    ...
    """
    # Filter vertices out of the polygon's bounding box, this serve as an early
    # optimization whenever number of vertices to filter out is huge.
    ...
    selection_mask[np.where(selection_mask)] &= polygon_mask
...
def select(polygon_vertices, points):
    ...
    if polygon_vertices is not None:
        # Optimization: It's faster to convert lasso selection straight to
        # visual coordinates since there's generally less vertices;
...
def on_mouse_move(event):
    ...
    if abs(px - pp[0]) > MIN_MOVE_UPDATE_THRESHOLD or abs(py - pp[1]) > MIN_MOVE_UPDATE_THRESHOLD:
        ...
        if event.button == 1:
            ...
            lasso.set_data(
                pos=np.insert(polygon_vertices, len(polygon_vertices), polygon_vertices[0], axis=0)
            )
            px, py = pp
