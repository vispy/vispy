...
l2 = scene.visuals.Tube(points2, color=["red", "green", "blue"], shading="smooth", tube_points=8)

l3 = scene.visuals.Tube(points3, color=colors, shading="flat", tube_points=8, closed=True)

l4 = scene.visuals.Tube(points4, color=colors, shading="smooth", tube_points=8, mode="lines")

# generate sine wave radii
radii = np.sin(2 * np.pi * 440 * np.arange(points5.shape[0]) / 44000)
radii = (radii + 1.5) / 2

l5 = scene.visuals.Tube(
    points5, radius=radii, color="white", shading="smooth", closed=True, tube_points=8
)
