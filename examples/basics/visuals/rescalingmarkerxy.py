# -*- coding: utf-8 -*-
# vispy: gallery 30
# -----------------------------------------------------------------------------
# Copyright (c) 2014, Vispy Development Team. All Rights Reserved.
# Distributed under the (new) BSD License. See LICENSE.txt for more info.
# -----------------------------------------------------------------------------
""" Display markers at different sizes and line thicknessess.
"""

import numpy as np

import vispy.scene as scene
from vispy.scene import visuals
import vispy.visuals as impl_visuals
from vispy import app

n = 540
pos = np.zeros((n, 2))
colors = np.ones((n, 4), dtype=np.float32)
radius, theta, dtheta = 1.0, 0.0, 5.5 / 180.0 * np.pi
for i in range(500):
    theta += dtheta
    x = radius * np.cos(theta)
    y = radius * np.sin(theta)
    r = 10.1 - i * 0.02
    radius -= 0.45
    pos[i] = x/512.+.5, y/512.+.5

nice_blue = (0.12572087695201239, 0.47323337360924367, 0.707327968232772, 1.)

class MyCanvas(scene.SceneCanvas):

    def __init__(self):
        scene.SceneCanvas.__init__(self, keys='interactive', size=(512, 512),
                                         title="Marker demo [press space to change marker]", bgcolor='white')
        self.index = 0
        self.markers = visuals.RescalingXYRelativeEdgeMarkers()
        self.markers.set_data(pos, face_color=nice_blue)
        self.markers.set_style(impl_visuals.markerxy_types[self.index])
        self.text = visuals.Text(impl_visuals.markerxy_types[self.index], pos=(80, 15), font_size=14, 
                                 color='black', parent=self.scene)


    def on_key_press(self, event):
        if event.text == ' ':
            self.index = (self.index + 1) % (len(impl_visuals.markerxy_types))
            self.markers.set_style(impl_visuals.markerxy_types[self.index])
            self.text.text = impl_visuals.markerxy_types[self.index]
            self.update()

if __name__ == '__main__':
    canvas = MyCanvas()
    grid = canvas.central_widget.add_grid()
    vb1 = grid.add_view(row=0, col=0)
    vb1.add(canvas.markers)    
    canvas.show()
    app.run()
