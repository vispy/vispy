...
thetas, phis = np.meshgrid(np.linspace(0, np.pi, 100), np.linspace(0, 2 * np.pi, 150))
shape = thetas.shape
...
for harm_degree in range(3):
    for harm_order in range(harm_degree + 1):
        ...
        scale = 1.0 / np.max(np.abs([v_min, v_max]))

        harmonic_values *= 0.5 * scale
        harmonic_values += 0.5
        colors = cm[harmonic_values.reshape(linear_shape)].rgba.reshape((shape[0], shape[1], 4))
